...
Stellite is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both Stellite and Bitcoin donations can be made to **donate.getstellite.org** if using a client that supports the [OpenAlias](https://openalias.org) standard. Alternatively you can send XMR to the Stellite donation address via the `donate` command (type `help` in the command-line wallet for details).
...
* Install the dependencies for Stellite from the 'Debian' column in the table above.
...
If you are using the older Raspbian Jessie image, compiling Stellite is a bit more complicated. The version of Boost available in the Debian Jessie repositories is too old to use with Stellite, and thus you must compile a newer version yourself. The following explains the extra steps, and has been tested on a Raspberry Pi 2 with a clean install of minimal Raspbian Jessie.
...
* Then, install the dependencies for Stellite except `libunwind` and `libboost-all-dev`
...
* From here, follow the [general Raspberry Pi instructions](#on-the-raspberry-pi) from the "Clone stellite and checkout most recent release version" step.
...
* If you would like a specific [version/tag](https://github.com/stellite-project/stellite/tags), do a git checkout for that version. eg. 'v0.12.3.0'. If you dont care about the version and just want binaries from master, skip this step:
...
The project can be built from scratch by following instructions for Linux above. If you are running stellite in a jail you need to add the flag: `allow.sysvipc=1` to your jail configuration, otherwise lmdb will throw the error message: `Failed to open lmdb environment: Function not implemented`.

We expect to add Stellite into the ports tree in the near future, which will aid in managing installations using ports or packages.
...
You will have to add the serialization, date_time, and regex modules to Boost when building as they are needed by Stellite.
...
Build stellite: `env DEVELOPER_LOCAL_TOOLS=1 BOOST_ROOT=/usr/local make release-static`
...
### On Linux for Android (using docker):
        ...
        cd utils/build_scripts/ && docker build -f android32.Dockerfile -t stellite-android .
        # Create container
        docker create -it --name stellite-android stellite-android bash
        # Get binaries
        docker cp stellite-android:/opt/android/stellite/build/release/bin .
...
## Installing Stellite from a package
...
* Ubuntu and [snap supported](https://snapcraft.io/docs/core/install) systems, via a community contributed build.
    ...
    snap install stellite --beta

Installing a snap is very quick. Snaps are secure. They are isolated with all of their dependencies. Snaps also auto update when a new version is released.

* Arch Linux (via [AUR](https://aur.archlinux.org/)):
  - Stable release: [`stellite`](https://aur.archlinux.org/packages/stellite)
  - Bleeding edge: [`stellite-git`](https://aur.archlinux.org/packages/stellite-git)

* Void Linux:

        xbps-install -S stellite

* GuixSD

        guix package -i stellite

* Docker

        # Build using all available cores
        docker build -t stellite .

        # or build using a specific number of cores (reduce RAM requirement)
        docker build --build-arg NPROC=1 -t stellite .

        # either run in foreground
        docker run -it -v /stellite/chain:/root/.bitstellite -v /stellite/wallet:/wallet -p 18080:18080 stellite

        # or in background
        docker run -it -d -v /stellite/chain:/root/.bitstellite -v /stellite/wallet:/wallet -p 18080:18080 stellite
...
[stellited.service](utils/systemd/stellited.service) to `/etc/systemd/system/` and
[stellited.conf](utils/conf/stellited.conf) to `/etc/`. The [example
service](utils/systemd/stellited.service) assumes that the user `stellite` exists
and its home is the data directory specified in the [example
config](utils/conf/stellited.conf).
...
While Stellite isn't made to integrate with Tor, it can be used wrapped with torsocks, by
setting the following configuration parameters and environment variables:

* `--p2p-bind-ip 127.0.0.1` on the command line or `p2p-bind-ip=127.0.0.1` in
  stellited.conf to disable listening for connections on external interfaces.
* `--no-igd` on the command line or `no-igd=1` in stellited.conf to disable IGD
  (UPnP port forwarding negotiation), which is pointless with Tor.
* `DNS_PUBLIC=tcp` or `DNS_PUBLIC=tcp://x.x.x.x` where x.x.x.x is the IP of the
  desired DNS server, for DNS requests to go over TCP, so that they are routed
  through Tor. When IP is not specified, stellited uses the default list of
  servers defined in [src/common/dns_utils.cpp](src/common/dns_utils.cpp).
* `TORSOCKS_ALLOW_INBOUND=1` to tell torsocks to allow stellited to bind to interfaces
...
* Do NOT pass `--detach` when running through torsocks with systemd, (see
  [utils/systemd/stellited.service](utils/systemd/stellited.service) for details).
* If you use the wallet with a Tor daemon via the loopback IP (eg, 127.0.0.1:9050),
  then use `--untrusted-daemon` unless it is your own hidden service.

Example command line to start stellited through Tor:

    DNS_PUBLIC=tcp torsocks stellited --p2p-bind-ip 127.0.0.1 --no-igd
...
allow inbound connections. Full example:
    ...
    DNS_PUBLIC=tcp torsocks ./stellited --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 \
        --data-dir /home/amnesia/Persistent/your/directory/to/the/blockchain

## Debugging

This section contains general instructions for debugging failed installs or problems encountered with Stellite. First ensure you are running the latest version built from the Github repo.
...
```
gdb /path/to/stellited `pidof stellited` 
```
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as stellited. It may be named just `core`, or `core.xxxx` with numbers appended.

You can now analyse this core dump with `gdb` as follows:

`gdb /path/to/stellited /path/to/dumpfile`

Print the stack trace with `bt`

* To run stellite within gdb:

Type `gdb /path/to/stellited`

Pass command-line options with `--args` followed by the relevant arguments

Type `run` to run stellited

### Analysing memory corruption

We use the tool `valgrind` for this.

Run with `valgrind /path/to/stellited`. It will be slow.
...
`cd ~/stellite/external/db_drivers/liblmdb && make`
